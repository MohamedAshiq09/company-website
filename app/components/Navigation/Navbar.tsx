...
// MobileNav Component
const MobileNav = ({ nav, closeNav }: { nav: boolean; closeNav: () => void; }) => {
...
// Nav Component
function Nav({ openNav }: { openNav: () => void; }) {
    ...
    <div className="h-[12vh] bg-gray-300 shadow-md w-full max-w-7xl mx-auto rounded-b-3xl px-4 sm:px-6 lg:px-8 fixed top-0 left-0 right-0 z-50">
      <div className="w-[85%] flex items-center h-[12vh] justify-between mx-auto">
        ...
        <ul className="hidden lg:flex items-center space-x-10">
          ...
          <li className="text-[18px] cursor-pointer font-bold text-red-400 hover:text-red-600 transition-all duration-200">
            <Link href={"/"}>Contact</Link>
          </li>
          <li className="text-[18px] cursor-pointer font-bold text-red-400 hover:text-red-600 transition-all duration-200">
            <Link href={"/"}>Blog</Link>
        ...
        <Bars3BottomRightIcon onClick={openNav} className="w-[2rem] h-[2rem] cursor-pointer text-slate-800" />
      </div>
    </div>
  );
}
